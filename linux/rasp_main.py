...
faceCascade = cv2.CascadeClassifier('./haarcascade_models/haarcascade_frontalface_default.xml')
eyeCascade = cv2.CascadeClassifier('./haarcascade_models/haarcascade_eye.xml')
...
def streaming():
    ...
    while True:
        ...
        '''
        gray = cv2.cvtColor(mirror_image, cv2.COLOR_BGR2GRAY)
        #gray변수에서 얼굴 데이터를 추출
        ...
        for (x,y,w,h) in faces:
            ...
            cv2.rectangle(mirror_image,(x,y),(x+w,y+h),(255,0,0),2)
            # 얼굴에 그려진 사각형 안쪽의 범위를 지정
            roi_gray = gray[y:y+h, x:x+w]
            roi_color = mirror_image[y:y+h, x:x+w]
            ...
            for (ex, ey, ew, eh) in eyes:
                ...
                (0,255,0), 2)
        if success:
            cv2.imshow("Help_Me_Hongs", mirror_image)
            key = cv2.waitKey(1) 
            if key == ord("q"):
