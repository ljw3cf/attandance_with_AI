...
def lambda_handler(event, context):
    ...
    if IMAGE_OBJECT_KEY[:2] == INDEX:
        ...
        temp_key = "TEMP-%s.json" % IMAGE_OBJECT_KEY
        json_data = S3.get_object(Bucket=BUCKET_NAME, Key=temp_key)
        ...
        if check_in_result == 0:
            add_check = ("INSERT INTO Check_in_out  "
                           ...
                           "VALUES ('%s', '%s', '%s')" %(SYSTEM_TIME, Check_In_Temp, Student_Id))
            ci_cur.execute(add_check)
            CONN.commit()

            RESULTS = "%s 님, 출석 완료되었습니다. %s 님의 온도는 %s입니다." % (Student_Name, Student_Name, Check_In_Temp)
            MESSAGE = {
                "Text": RESULTS
            }
            JSON_PATH = "/tmp/%s.json" % IMAGE_OBJECT_KEY
        ...
        # 출석기록이 존재하는 경우
            ...
            # 출석기록 존재하나, 퇴실기록 없는 경우
                ...
                RESULTS = "%s 님, 퇴실 완료되었습니다. %s 님의 온도는 %s입니다." % (Student_Name, Student_Name, Check_Out_Temp)
                MESSAGE = {
                    "Text": RESULTS
                }
                JSON_PATH = "/tmp/%s.json" % IMAGE_OBJECT_KEY
